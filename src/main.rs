...
fn main() -> GameResult {
    ...
    let cb = ContextBuilder::new("sushye", "esehara shigeo")
        ...
        .window_mode(conf::WindowMode::default().dimensions(
            (WINDOWSIZE_WIDTH * TILESIZE) as f32,
            (WINDOWSIZE_HEIGHT * TILESIZE) as f32,
        ));
...
impl State {
    ...
    pub fn enum_to_unicode(self: &State, refer: &Renderable) -> graphics::TextFragment {
        match refer.image {
            GameImage::Player => {
                graphics::TextFragment::new("＠").color(graphics::Color::new(1.0, 1.0, 0.0, 1.0))
            }
            GameImage::Dragon => {
                graphics::TextFragment::new("龍").color(graphics::Color::new(1.0, 0.1, 0.1, 1.0))
            }
            GameImage::Kobold => {
                graphics::TextFragment::new("妖").color(graphics::Color::new(0.0, 1.0, 0.0, 1.0))
            }
            GameImage::Potion => {
                graphics::TextFragment::new("壺").color(graphics::Color::new(0.0, 0.5, 1.0, 1.0))
            }
            GameImage::Scroll => {
                graphics::TextFragment::new("紙").color(graphics::Color::new(1.0, 0.9, 0.7, 1.0))
            }
            GameImage::Sword => {
                graphics::TextFragment::new("剣").color(graphics::Color::new(0.5, 0.5, 0.0, 1.0))
            }
            GameImage::Shield => {
                graphics::TextFragment::new("盾").color(graphics::Color::new(0.0, 0.0, 0.8, 1.0))
            }
            _ => graphics::TextFragment::new("謎").color(graphics::Color::new(1.0, 1.0, 1.0, 1.0)),
        }
    }
    ...
    pub fn new(ctx: &mut Context, hidpi_factor: f32) -> GameResult<State> {
        ...
        // loading Image
        prepare_images.insert(
            GameImage::Player,
            graphics::Image::new(ctx, "/player.png").unwrap(),
        );
        prepare_images.insert(
            GameImage::Dragon,
            graphics::Image::new(ctx, "/ice_dragon_new.png").unwrap(),
        );
        prepare_images.insert(
            GameImage::Kobold,
            graphics::Image::new(ctx, "/kobold.png").unwrap(),
        );
        prepare_images.insert(
            GameImage::Wall,
            graphics::Image::new(ctx, "/brick.png").unwrap(),
        );
        prepare_images.insert(
            GameImage::Floor,
            graphics::Image::new(ctx, "/floor.png").unwrap(),
        );
        prepare_images.insert(
            GameImage::DownStairs,
            graphics::Image::new(ctx, "/downstairs.png").unwrap(),
        );
        prepare_images.insert(
            GameImage::Potion,
            graphics::Image::new(ctx, "/potion.png").unwrap(),
        );
        prepare_images.insert(
            GameImage::Scroll,
            graphics::Image::new(ctx, "/scroll.png").unwrap(),
        );
        prepare_images.insert(
            GameImage::Sword,
            graphics::Image::new(ctx, "/sword.png").unwrap(),
        );
        prepare_images.insert(
            GameImage::Shield,
            graphics::Image::new(ctx, "/shield.png").unwrap(),
        );
                ...
                for x in 0..PLAYER_WINDOW_WIDTH {
                    ...
                    if map.revealed_tiles[idx] {
                        if self.render_mode == RenderMode::Tile {
                            graphics::draw(
                                ctx,
                                self.images.get(&tile.tiletype.to_game_image()).unwrap(),
                            ...
                            match tile.background {
                                ...
                                Some(color) => {
                                    let background_rect = graphics::Mesh::new_rectangle(
                                        ...
                                        graphics::Rect::new(
                                            ui_helper::map_to_p(x),
                                            ui_helper::map_to_p(y),
                                            TILESIZE as f32,
                                            TILESIZE as f32,
                                        ),
                                        color,
                            ...
                            match tile.tiletype {
                                TileType::Floor => {
                                    use_text = graphics::TextFragment::new("床")
                                        .color(graphics::Color::new(0.1, 0.1, 0.1, 0.4));
                                }
                                TileType::Wall => {
                                ...
                                TileType::DownStairs => {
                                    use_text = graphics::TextFragment::new("門")
...
impl EventHandler for State {
    ...
    fn key_down_event(
        ...
        match newrunstate {
                        ...
                        match keycode {
                            ...
                            _ => {
                                ...
                                newrunstate = try_use_item(
                                    &mut self.ecs,
                                    (keycode as i32) - 10,
                                    &mut self.imgui.inventory_window_show,
                                );
                            }
                        }
                    }
            ...
            RunState::ShowTargeting { range: _, item: _ } => match keymod {
                KeyMods::NONE => match keycode {
                    KeyCode::Escape => newrunstate = RunState::AwaitingInput,
                    _ => {}
                },
                _ => {}
            },
            RunState::AwaitingInput => match keymod {
                KeyMods::SHIFT => match keycode {
